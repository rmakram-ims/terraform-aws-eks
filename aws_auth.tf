...
resource "null_resource" "update_config_map_aws_auth" {
  ...
  provisioner "local-exec" {
    command = "kubectl apply -f ${var.config_output_path}config-map-aws-auth_${var.cluster_name}.yaml --kubeconfig ${var.config_output_path}kubeconfig_${var.cluster_name} --request-timeout ${var.kubectl_request_timeout}"
  }
